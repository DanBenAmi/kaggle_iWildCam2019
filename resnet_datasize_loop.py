...
class WildDataset(Dataset):
    def __init__(self, df, img_dir, augs=None, mode=True):
        # self.df = df[:200]
        ...
        if mode:
            ...
            # self.df = self.df.sample(frac=1, random_state=1).reset_index()
...
train, val = train_test_split(train_df, stratify=train_df.category_new_id, test_size=0.01)
...
def train_model(name, epochs, model, train_loader, val_loader, optimizer, scheduler=None,
    ...
    for epoch in range(epochs):
        ...
        if result['val_f1'] > best_f1:
            ...
            torch.save(model, ckpts_dir + '/{}_trained_datasize-{}_epoch-{}_f1-{:.4f}_date-{}.pt'.format(name,
                        len(train_loader.dl.dataset.df), epoch, best_f1, date.today()))
...
for i in range(0, 7):
    ...
    curr_ds = WildDataset(df=train_set.df[:ds_length], img_dir=train_dir, augs=aug, mode=False)
    train_loader = DataLoader(dataset=curr_ds, batch_size=50, sampler=curr_ds.sampler)
    ...
    with open(f"{ckpts_dir}/history_ResNet_FT_{ds_length}.pkl", 'wb') as fp:
